...
let aztecNode: unknown = null;
let isShuttingDown = false;

// Account storage (in-memory for now, synced with Rust DB)
interface StoredAccount {
  id: string;
  evmAccountId: string;
  aztecAddress: string;
  partialAddress: string;
  privacySecretEnc: string;
...
const storedAccounts: Map<string, StoredAccount> = new Map();
...
let aztecModules: Record<string, unknown> = {};

async function initAztecModules() {
  if (Object.keys(aztecModules).length > 0) return aztecModules;
...
async function getAztecNode(rpcUrl: string) {
  ...
  const { createAztecNodeClient } = aztecModules as {
  ...
  aztecNode = node;
  (aztecNode as { url?: string }).url = rpcUrl;
...
const handlers: Record<string, (params: Record<string, unknown>) => Promise<unknown>> = {
  ...
  async 'system.sdkTest'(_params) {
    ...
    try {
      ...
      log('ERROR', 'SDK initialization failed', { error: String(error), stack: (error as Error).stack });
      return {
        success: false,
        error: String(error),
  ...
  async 'node.connect'(params) {
    const { rpcUrl } = params as { rpcUrl: string };
    if (!rpcUrl) {
  ...
  async 'node.getInfo'(params) {
    ...
    const info = await (node as { getNodeInfo: () => Promise<unknown> }).getNodeInfo();
    return info;
  },
  ...
  async 'account.getTestAccounts'(params) {
    ...
    const { getInitialTestAccountsData } = aztecModules as {
      getInitialTestAccountsData: (node: unknown) => Promise<unknown[]>;
    };
    
    const accounts = await getInitialTestAccountsData(node);
    return accounts.map((acc: unknown) => ({
      address: (acc as { address: { toString: () => string } }).address.toString(),
      secret: (acc as { secret: { toString: () => string } }).secret.toString(),
    }));
  },
  ...
  async 'account.save'(params) {
    const account = params as unknown as StoredAccount;
  ...
  async 'account.getDeployed'(params) {
    const { address } = params as { address: string };
    const account = Array.from(storedAccounts.values()).find(
      acc => acc.aztecAddress.toLowerCase() === address.toLowerCase()
    );
    return account?.isDeployed ?? false;
  },
  
  async 'transfer.build'(params) {
    const { fromAddress, toAddress, amount, asset } = params as {
      fromAddress: string;
      toAddress: string;
      amount: string;
      asset?: string;
  ...
  async 'transfer.send'(params) {
    const { signedTx } = params as { signedTx: string };
  ...
  async 'balance.get'(params) {
    const { address, asset } = params as { address: string; asset?: string };
...
export async function startServer(port = PORT): Promise<{ port: number; token: string }> {
  if (server) {
