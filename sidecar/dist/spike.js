 ...
 */
import { createAztecNodeClient, AccountManager, Fr, FEE_JUICE_ADDRESS } from '@aztec/aztec.js';
import { EcdsaKAccountContract, getInitialTestAccountsData } from '@aztec/accounts';
// ============================================
// CONFIGURATION
// ============================================
...
async function spike2_createAccountWithEVMKey(node, evmPrivateKeyHex) {
    ...
    try {
        ...
        const accountManager = await AccountManager.create(node, privacySecret, ecdsaAccount, salt);
        ...
        log('ERROR', 'Failed to create account', { error: String(error), stack: error.stack });
        return { success: false, error };
    }
}
async function spike3_deployAccount(accountManager, completeAddress) {
...
async function spike4_checkBalance(node, address) {
...
async function spike5_testWithTestAccounts(node) {
