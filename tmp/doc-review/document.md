...
- **Idle auto-lock is backend-driven with a frontend heartbeat.** A tokio timer in the backend clears the encryption key after the idle timeout. The frontend resets the timer with a lightweight heartbeat during user activity and on every sensitive command. This keeps the source of truth in the Rust layer, which is the only layer that can know whether the key is still in memory.
...
3. Decrypt the sentinel value with the legacy key; if it fails, reject the PIN.
4. Generate a new random salt, derive the new key, and persist salt + version marker.
...
**Approach:**
- Keep a small encrypted sentinel value (e.g., the nsec stored as `pkey`) in `settings` after the first successful encrypt. If no sentinel exists, use the `pkey` value itself as the sentinel.
...
- Emit `migration_complete` on success.
...
**Patterns to follow:**
- The existing `ENCRYPTION_KEY` global is set exactly once after PIN validation. Keep the same set-once semantics but wrap the value in the secret container.
...
- Store the default timeout as a constant (15 minutes) and make it overridable via a setting later if needed (out of scope for this pass).
...
- Before every sensitive command (send message, sign transaction, export key), call `check_session` and abort if locked.
- On `session_locked`, set `isAuthenticated=false` and `currentUser=null` and navigate to the unlock screen.
...
- Add `sensitive_export_log` table with columns: `id`, `account_npub`, `export_type`, `attempted_at` (epoch seconds), `success` (boolean), `error_code` (optional).
- On app shutdown, clear any active clipboard timer and the clipboard if an export is still pending.
...
**Approach:**
- Replace the two-phase `pin -> key` flow with a `confirm -> pin -> copied` flow.
...
- For "Export all", make separate backend calls for nsec, seed, and each EVM account; the backend logs each independently.
...
**Requirements:** R5, R6 (partial), AE5.
...
- Apply the guard to `export_sensitive_to_clipboard`, `sign_evm_hash`, `message` sending, and any squad-admin write commands that should require the new key derivation.
- On the frontend, if the guard returns an error, show the unlock screen so the user can trigger the migration on the next PIN entry.
