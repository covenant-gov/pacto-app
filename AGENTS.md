...
- **Before UI, theme, or shell work:** read [`docs/design-system/`](docs/design-system/) (start with `README.md`, then `THEMING.md` / `SHELL.md` / `UI.md` as needed). Follow those rules; do not import React/Next defaults.
...
- **Before changing SQLite schema:** run `make new-migration name=<snake_case_description>` to create a new refinery migration under `src-tauri/src/migrations/` with a UTC-timestamp version (`V<YYYYMMDDHHMMSS>__snake_case_name.sql`), then write the SQL; or add a `.rs` migration if the transformation requires generated SQL. Timestamp versions — not sequential integers — because two branches under parallel development independently computing "the next number" collide on `refinery_schema_history`'s version primary key the moment both merge; a UTC second-precision timestamp makes that collision require two migrations authored in the same second. Migrations up to `V32` keep sequential names (`V31`/`V32` are reserved for #233 and #235, already in flight with hand-picked filenames when this convention landed); do not renumber history. Two backstops run in `cargo test --lib` (`src-tauri/src/storage_format.rs`'s `embedded_set_matches_committed_migration_files`) so a bypass of `make new-migration` still fails in CI: no two migration files may share a version, and no version above `V32` may look like a hand-typed sequential integer instead of a timestamp. Do not edit inline DDL in tests; tests should apply the same migration set via `crate::migrations::run_migrations`.
...
- **Before creating or updating a PR:** use the [`value-based-pr`](.agents/skills/value-based-pr/README.md) skill so the title and description explain what changed and why it matters, not just which files were touched. When the PR closes multiple GitHub issues, use one `closes #n` per issue ([`github-pr-closes-issues`](.agents/skills/github-pr-closes-issues/SKILL.md)); `closes #n0 #n1` will not close `#n1`.
