...
- **Before changing SQLite schema:** add a new refinery migration under `src-tauri/src/migrations/` named with a UTC timestamp version, `V<YYYYMMDDHHMMSS>__snake_case_name.sql` (e.g. `` `date -u +V%Y%m%d%H%M%S` ``, giving `V20260808213000__my_change.sql`), or a `.rs` migration if the transformation requires generated SQL. Timestamp versions — not sequential integers — because two branches under parallel development independently computing "the next number" collide on `refinery_schema_history`'s version primary key the moment both merge; a UTC second-precision timestamp makes that collision require two migrations authored in the same second. Pre-2026-08-08 migrations keep their sequential `V1`..`V30` names; do not renumber history. `src-tauri/src/storage_format.rs`'s `embedded_set_matches_committed_migration_files` test fails the build if two migration files ever share a version, so a collision surfaces in CI, not at migration-apply time in production. Do not edit inline DDL in tests; tests should apply the same migration set via `crate::migrations::run_migrations`.
