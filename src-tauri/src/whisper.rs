...
pub const MODELS: [WhisperModel; 5] = [
    WhisperModel {
        name: "tiny",
        display_name: "Lowest Quality - Fastest",
        size: 75,
    },
    WhisperModel {
        name: "base",
        display_name: "Low Quality - Faster",
        size: 142,
    },
    WhisperModel {
        name: "small",
        display_name: "Base Quality - Fast",
        size: 466,
    },
    WhisperModel {
        name: "medium",
        display_name: "High Quality - Slow",
        size: 1500,
    },
    WhisperModel {
        name: "large-v3",
        display_name: "Highest Quality - Slowest",
        size: 2900,
    },
];
...
pub fn is_model_downloaded<R: Runtime>(handle: &AppHandle<R>, model_name: &str) -> bool {
    ...
    let models_dir = crate::test_sandbox::test_local_data_dir(handle)
        .unwrap()
        .join("whisper");
...
pub async fn download_whisper_model<R: Runtime>(
    ...
    println!(
        "Downloading {} model (~{}MB), please wait...",
    ...
    let response = match response {
        ...
        None => {
            let error_msg = last_error
                .unwrap_or_else(|| "Failed to download model from all sources".to_string());
        ...
        match chunk_result {
            Ok(chunk) => {
                ...
                if total_size > 0 {
                    ...
                    print!(
                        "\rDownloading: {}% ({}/{} bytes, {} chunks)",
                        percent, downloaded, total_size, chunk_count
                    );
                    io::stdout().flush()?;
                } else {
                    print!(
                        "\rDownloaded: {} MB ({} chunks)",
