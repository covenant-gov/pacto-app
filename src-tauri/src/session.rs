...
impl SessionManager {
    ...
    fn spawn_timer_locked(&self, state: &mut InnerState) {
        ...
        // Auto-lock is disabled when timeout is 0.
        if timeout_ms == 0 {
            return;
        }
...
/// Read a persisted idle timeout (in milliseconds) from an existing account
/// connection and apply it to the session manager. Falls back to the default
/// if the setting is missing or unreadable. A stored value of `0` disables
/// auto-lock.
...
/// Set the idle auto-lock timeout and persist it in account settings.
/// A value of `0` disables auto-lock.
#[tauri::command]
pub fn set_session_timeout(timeout_minutes: u64) {
    let timeout_ms = timeout_minutes * 60_000;
    SESSION_MANAGER.set_timeout_ms(timeout_ms);
    // Restart the timer so the new timeout takes effect immediately instead of
    // waiting for the previous timer to fire or the next user heartbeat.
    SESSION_MANAGER.heartbeat();
    if let Ok(handle) = crate::TAURI_APP.get().ok_or("Tauri app not initialized") {
        if let Ok(conn) = account_manager::get_db_connection(handle) {
            let sql = "INSERT INTO settings (key, value) VALUES ('session_idle_timeout_ms', ?1)
                       ON CONFLICT(key) DO UPDATE SET value = excluded.value";
            let _ = conn.execute(sql, rusqlite::params![timeout_ms.to_string()]);
            account_manager::return_db_connection(conn);
        }
    }
}

/// Return the persisted idle timeout in minutes, or the default. A stored
/// value of `0` means auto-lock is disabled.
#[tauri::command]
pub fn get_session_timeout() -> u64 {
    let default_minutes = DEFAULT_IDLE_TIMEOUT_MS / 60_000;
    let Ok(handle) = crate::TAURI_APP.get().ok_or("Tauri app not initialized") else {
...
mod tests {
    ...
    #[tokio::test]
    async fn disabled_auto_lock_never_fires() {
        let manager = SessionManager::new();
        manager.set_timeout_ms(0);
        manager.set_key([5u8; 32]);
        assert!(manager.is_unlocked());

        sleep(TokioDuration::from_millis(150)).await;
        assert!(
            manager.is_unlocked(),
            "session should stay unlocked when auto-lock is disabled"
        );
    }
