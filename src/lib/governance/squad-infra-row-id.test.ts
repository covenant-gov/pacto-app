...
describe('squad infra row ids', () => {
  it('uses direct prefix for short parent ids', () => {
    ...
    expect(pactoGovWargameInfraId(SHORT_PARENT)).toBe(`pacto-gov-wargame-${SHORT_PARENT}`);
  ...
  it('hashes long parent ids to stay within SQLite limit', () => {
    ...
    const wargameId = pactoGovWargameInfraId(LONG_PARENT);
    const sponsorId = squadSponsorInfraId(LONG_PARENT);
    const adminId = squadAdminInfraId(LONG_PARENT);

    for (const id of [govId, treasuryId, wargameId, sponsorId, adminId]) {
    ...
    expect(wargameId).toMatch(/^pgw-[a-f0-9]{64}$/);
