<script lang="ts">
	...
	let {
		...
		pollTitleFor?: (token: string) => string | undefined;
	} = $props();

	function tokenize(value: string): TextToken[] {
		const tokens: TextToken[] = [];
		const pattern = /(@[\w.-]+|#poll-\d+|#\d+|#[\w-]+)/g;
		let last = 0;
		...
		while ((match = pattern.exec(value)) !== null) {
			...
			else if (/^#poll-\d+$/.test(token)) tokens.push({ kind: 'poll', value: token });
...
{#each tokens as token, i (i)}
	...
	{:else if token.kind === 'poll'}
		{@const title = pollTitleFor?.(token.value) ?? $t('design.chat.pollFallback')}
		<span
			class="inline-flex max-w-[min(100%,20rem)] items-center gap-1 rounded-sm bg-primary/22 px-1.5 py-0.5 align-baseline text-[0.92em] leading-none font-medium text-mention-accent shadow-[inset_0_0_0_1px_color-mix(in_srgb,var(--mention-accent)_28%,transparent)]"
		>
			<span class="shrink-0 font-semibold tabular-nums opacity-80">{token.value}</span>
			<span class="min-w-0 truncate">{title}</span>
		</span>
