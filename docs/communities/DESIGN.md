...
Roster bindings (`squad_member_evm_account`, `squad_member_evm`) and on-chain infra (`squad_infra`, `parent_treasury_safe`) are also SQLite; keyed by the same `parent_id`. Squad-tracked ERC-20 rows for Treasury Safe balance UI live in `squad_tracked_tokens` (additive; MLS announce sync). Hats/roles and process-board display slices live in `squad_gov_replica` (MLS snapshot; not ACL).
...
**Late joiners / catch-up:** MLS Kind-444 backfill is bounded (~48h). When a member joins `#announcements`, the client auto-publishes `squad_state_sync_request` with `requested: ['evm','infra','network','rpc','channels','identity','gov_replica']`. Online peers republish network, announcements-scoped `governance_updated`, RPC config, an open-channel **catalog** (`squad_channels_catalog`), and MLS-admit the requester into any **open** custom channels they are missing. Peers republish roster EVM **only if** they have a `squad_member_evm_account` binding for that parent; unbound peers omit EVM and still help with infra/network/channels. Peers republish squad identity (`squad_identity_updated`) **only if** they have a local `iconUrl`; empty local icons are not announced, so a late joiner cannot wipe the squad photo. Anyone can also press **Request sync** on My Dashboard → Status (same request). Hats/roles and TA/QM process rows persist in `squad_gov_replica` (keyed by live vs war-game stack). `governance_process_updated` may carry a bounded snapshot; ingest is last-write-wins on `block_number`. Peers republish the last SQLite replica — they do not RPC to answer a late joiner. Empty replica: the opener does one chain fill and upserts locally. Writes stay fail-closed on chain (`require_capability`); the replica is display + catch-up only. ACL UI still self-checks the viewer via `get_squad_capabilities`.
