...
- KTD1. Bind `relay_url` in `notifs()`'s `handle_notifications` closure (`src-tauri/src/lib.rs`) off `RelayPoolNotification::Message { relay_url, message: RelayMessage::Event { event, .. } }` (not `RelayPoolNotification::Event`, which the `nostr-relay-pool-0.43.1` crate documents as pool-wide deduplicated — sent only the first time a given event ID is seen — which would undercount `events_received`/`bytes_down` for every relay that isn't first to deliver a given event) and call a new `record_event_received(relay_url, event)` helper that increments `events_received` and adds `event.as_json().len()` to `bytes_down` via the existing `update_relay_metrics`. Runs unconditionally, as an additional branch alongside the existing `RelayPoolNotification::Event` arm (left unchanged for its correct one-time-per-event DM/MLS processing), so it covers every per-relay delivery regardless of subscription or dedup. Governs R5, R7.
