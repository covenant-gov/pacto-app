...
`personal-alerts` and `join-requests` are **not** sidebar channels. MLS buckets `inbox` and `join_requests` still exist for transport; UI surfaces them under **My Dashboard → Alerts** and **`#settings` → Join requests**.
...
`#my-dashboard` is a single stacked page: checklist, Alerts, roster EVM, then squad sync.
