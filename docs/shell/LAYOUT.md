...
**Pinned order:** `squad-dashboard` → `squad-wargame` (when Active) → `announcements` → `polls` → `settings`

`personal-alerts` and `join-requests` are **not** sidebar channels. MLS buckets `inbox` and `join_requests` still exist for transport; UI surfaces them under **`#settings` → username mode → Alerts** and **`#settings` → Squad mode → Join requests**. Persisted `__my_dashboard__` remaps to `#settings`.
...
| Module | Owns |
|--------|------|
| `navigation.ts` | Top nav, squad/channel selection, squad dashboard and settings channel modes, last-opened maps |
...
**Keep-alive:** After a Squad Dashboard mode is visited once, `ParentDashboard` keeps that tab mounted and toggles visibility with `hidden`/CSS so form and sub-mode state survive mode switches (avoids remount races on Mutiny / QM / Safe loaders). `#settings` uses the same keep-alive for username and Squad panes.
