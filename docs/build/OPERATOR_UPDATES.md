...
- **Ad-hoc signed builds and Gatekeeper:** macOS builds are ad-hoc signed (`signingIdentity: "-"` in `src-tauri/tauri.conf.json`) — this satisfies Apple Silicon's requirement that executables carry *some* signature, but it is not a Developer ID signature and is not notarized. Gatekeeper still treats the app as from an unidentified developer, and quarantine still applies to downloaded builds: removing the quarantine attribute before first launch is required, otherwise Gatekeeper translocation can cause the updater to modify a temporary copy instead of the app in `/Applications`. On Windows, unsigned installers may trigger SmartScreen; users should choose "More info" → "Run anyway".
