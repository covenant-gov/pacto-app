...
- [ ] **Once per chain:** fund shared **paymaster EntryPoint deposit** (`paymaster.deposit()` on `0xF7f557…` / see [PACTO_SQUAD_SPONSOR.md](./PACTO_SQUAD_SPONSOR.md)) — separate from squad pool deposits.
...
| `PAYMASTER_DEPOSIT_LOW` | Fund shared paymaster via `deposit()` / `EntryPoint.depositTo` — **not** the squad sponsor pool |
| `PAYMASTER_STAKE_LOW` | Paymaster needs EntryPoint `addStake` (owner/factory ops) |
